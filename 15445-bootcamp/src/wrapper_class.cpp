...
class IntPtrManager {
  public:
    ...
    ~IntPtrManager() {
      ...
      if (ptr_) { // although redundant as delete has builtin check for nullptr
        delete ptr_;
      }
    }
    ...
    IntPtrManager &operator=(IntPtrManager &&other) {
      if (ptr_ == other.ptr_) {
        return *this;
      } // This is a self assignment guard
      if (ptr_) {
        delete ptr_;
      } // just to delete old baggage.since this is a already existing object
      ptr_ = other.ptr_;
      other.ptr_ = nullptr;
      return *this; // returning a reference to the object on the left side.
                    // without this '=' chaining would break. 
    }
